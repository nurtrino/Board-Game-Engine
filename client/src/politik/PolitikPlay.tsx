...
interface CatalogEntry {
  ...
  industries?: (typeof INDUSTRIES)[number][];
  corruption?: boolean;
  negotiation?: boolean;
...
function PendingPrompt({ pending, view, me, scene, send, inspect, zoom, clashResume = false }: {
  ...
  const [showClashExceptionTools, setShowClashExceptionTools] = useState(false);
  const hand = me.hand ?? [];
  const verifiedEdgeAt = (card: { kind: string; id: string } | null | undefined, timing: NonNullable<CatalogEntry['edgeTimings']>[number]) => {
    const entry = card?.kind === 'politik' ? CATALOG[card.id] : null;
    return !!entry?.structureVerified && (entry.edgeTimings?.includes('at_any_time') || entry.edgeTimings?.includes(timing));
  };
  ...
  if (pending.kind === 'edge_window') {
    const edgeCards = hand.map((card, index) => ({ card, index, spec: playSpec(card) })).filter((item) => item.spec.kind === 'event' && verifiedEdgeAt(item.card, 'at_any_time'));
    const edgeAbilities = [
      ...me.propaganda.map((card) => ({ label: card.title, ready: card.ready, card: card.card, source: { kind: 'propaganda', id: card.instanceId } })),
      ...me.companies.map((card) => ({ label: card.title, ready: card.ready, card: card.card, source: { kind: 'company', id: card.id } })),
      ...me.companies.flatMap((company) => company.assets.map((asset) => ({ label: asset.title, ready: asset.ready, card: asset.card, source: { kind: 'asset', id: asset.instanceId } }))),
    ].filter((ability) => verifiedEdgeAt(ability.card, 'at_any_time'));
    return (
      <div className="pk-prompt-card pk-edge-prompt ig-glass">
        ...
        <div className="pk-edge-order">{pending.order.map((seat, index) => <span key={seat} className={index < pending.cursor ? 'passed' : index === pending.cursor ? 'current' : ''}><b>{index + 1}</b><small>{view.players[seat]?.name}</small></span>)}</div>
        {!!edgeCards.length && <><span className="ig-lab">VERIFIED EVENTS IN HAND</span><p>Only events whose reviewed print permits this timing are shown.</p><div className="pk-commit-hand">{edgeCards.map(({ card, index }) => <button key={`${card.id}-${index}`} onClick={() => inspect?.(card, index)}><PolitikCard scene={scene} card={handCardArt(card)} label={CATALOG[card.id]?.name ?? cardLabel(card, index)} /><span>{CATALOG[card.id]?.name ?? cardLabel(card, index)}<b>VIEW AND PLAY</b></span></button>)}</div></>}
  ...
  if (pending.kind === 'clash') {
    ...
    const clashTiming = pending.stage === 'after_cost' ? 'after_cost' : pending.stage === 'attacker_focus' || pending.stage === 'defender_focus' ? 'during_focus' : pending.stage === 'after_reveal' ? 'after_reveal' : 'before_resolve';
    const clashEdgeCards = hand.map((card, index) => ({ card, index, spec: playSpec(card) })).filter((item) => item.spec.kind === 'event' && verifiedEdgeAt(item.card, clashTiming));
    const clashEdgeAbilities = [
      ...me.propaganda.map((card) => ({ label: card.title, ready: card.ready, card: card.card, source: { kind: 'propaganda', id: card.instanceId } })),
      ...me.companies.map((card) => ({ label: card.title, ready: card.ready, card: card.card, source: { kind: 'company', id: card.id } })),
      ...me.companies.flatMap((company) => company.assets.map((asset) => ({ label: asset.title, ready: asset.ready, card: asset.card, source: { kind: 'asset', id: asset.instanceId } }))),
    ].filter((ability) => verifiedEdgeAt(ability.card, clashTiming));
    return (
      <div className="pk-prompt-card pk-clash-prompt ig-glass" data-testid="politik-clash-stage">
        ...
        {commitmentStage && mine ? <>
          <div className="pk-clash-privacy"><b>COMMIT IN SECRET</b><span>Select cards, then add leaders and legal adjacent Influence. Verified printed {arena} Focus is filled automatically; only an unverified future card asks for manual Focus. No opponent sees identities or totals before reveal.</span></div>
              ...
              return (
                <div className="pk-commit-card" key={`${card.id}-${index}`}>
                  <button className={on ? 'on' : ''} disabled={card.kind === 'obligation'} onClick={() => setSelected((list) => on ? list.filter((value) => value !== index) : [...list, index])}>
                    <PolitikCard scene={scene} card={handCardArt(card)} label={cardLabel(card, index)} />
                    <span>{catalog(card)?.name ?? cardLabel(card, index)} <b>{card.kind === 'obligation' ? 'NOT A FOCUS CARD' : card.kind === 'startup' ? 'UNIVERSAL 1 FOCUS' : `${focus[index] ?? printed} ${catalog(card)?.focusVerified ? 'VERIFIED ' : ''}FOCUS`}</b></span>
                  </button>
          ...
          {selected.filter((index) => hand[index]?.kind === 'politik' && !catalog(hand[index])?.focusVerified).map((index) => <ResourceStepper key={`focus-${index}`} label={`${catalog(hand[index])?.name ?? cardLabel(hand[index], index)} PRINTED ${arena.toUpperCase()} FOCUS · MANUAL FALLBACK`} value={focus[index] ?? printedFocus(hand[index])} max={10} onChange={(value) => setFocus((current) => ({ ...current, [index]: value }))} />)}
          ...
          <button className="pk-primary" data-testid="politik-clash-commit" onClick={() => send({ type: 'clash_commit', cards: selected.map((handIndex) => catalog(hand[handIndex])?.focusVerified ? { handIndex } : { handIndex, focus: focus[handIndex] ?? printedFocus(hand[handIndex]) }), leaders, focusInfluence })}>LOCK HIDDEN COMMITMENT</button>
        ...
        {responseStage && <><div className="pk-edge-order">{pending.order.map((seat, index) => <span key={seat} className={index < pending.cursor ? 'passed' : index === pending.cursor ? 'current' : ''}><b>{index + 1}</b><small>{view.players[seat]?.name}</small></span>)}</div>{mine ? <div className="pk-clash-response"><span className="ig-lab">PLAY AN ELIGIBLE EDGE OR PASS</span>{(clashEdgeCards.length > 0 || clashEdgeAbilities.length > 0) && <div className="pk-clash-edge-options"><div className="pk-card-targets">{clashEdgeCards.map(({ card, index }) => <button key={`${card.id}-${index}`} onClick={() => inspect?.(card, index)}><PolitikCard scene={scene} card={handCardArt(card)} label={CATALOG[card.id]?.name ?? cardLabel(card, index)} /><b>{CATALOG[card.id]?.name ?? cardLabel(card, index)}</b><small>EDGE EVENT · TAP TO INSPECT</small></button>)}{clashEdgeAbilities.map((ability) => <div className="pk-edge-ability-card" key={`${ability.source.kind}-${ability.source.id}`}><PolitikCard scene={scene} card={handCardArt(ability.card)} label={ability.label} /><b>{ability.label}</b><small>{ability.ready ? 'READY' : 'USED'} · PRINTED EDGE</small><div><button onClick={() => send({ type: 'use_ability', source: ability.source, asEdge: true, activate: false })}>USE</button><button disabled={!ability.ready} onClick={() => send({ type: 'use_ability', source: ability.source, asEdge: true, activate: true })}>{ability.ready ? 'ACTIVATE' : 'USED'}</button></div></div>)}</div></div>}<button className="pk-clash-exception-toggle" data-testid="politik-clash-exception-toggle" onClick={() => setShowClashExceptionTools((open) => !open)}>{showClashExceptionTools ? 'HIDE UNENCODED EXCEPTION' : 'RESOLVE AN UNENCODED PRINTED EXCEPTION'}</button>{showClashExceptionTools && <div className="pk-clash-exception-tools" data-testid="politik-clash-exception-tools"><p className="pk-clash-direct-note">Use these fields only when authentic printed text changes or cancels this Clash and that exact effect is not encoded yet.</p><div className="pk-clash-response-fields"><label>MODIFY SIDE<select data-testid="politik-clash-modifier-side" value={clashModifierSide} onChange={(event) => setClashModifierSide(event.target.value as typeof clashModifierSide)}><option value="attacker">{attacker?.name ?? 'ATTACKER'}</option><option value="defender">{defender?.name ?? 'IMPERIALS'}</option></select></label><label>SIGNED FOCUS<input data-testid="politik-clash-modifier-amount" type="number" value={clashModifierAmount} onChange={(event) => setClashModifierAmount(Number(event.target.value))} /></label><label>PRINTED SOURCE<input data-testid="politik-clash-modifier-source" value={clashModifierSource} maxLength={160} onChange={(event) => setClashModifierSource(event.target.value)} placeholder="Name the card or ability" /></label><button data-testid="politik-clash-modifier-submit" disabled={!clashModifierAmount || clashModifierSource.trim().length < 3} onClick={() => send({ type: 'clash_modifier', side: clashModifierSide, amount: clashModifierAmount, source: clashModifierSource.trim() })}>APPLY PRINTED MODIFIER</button></div><div className="pk-clash-cancel-row"><input data-testid="politik-clash-cancel-source" value={clashCancelSource} maxLength={160} onChange={(event) => setClashCancelSource(event.target.value)} placeholder="Printed Clash cancellation source" /><button data-testid="politik-clash-cancel-submit" disabled={clashCancelSource.trim().length < 3} onClick={() => send({ type: 'cancel_clash', source: clashCancelSource.trim() })}>CANCEL FROM PRINTED EFFECT</button></div></div>}<button className="pk-primary" data-testid="politik-clash-pass" onClick={() => send({ type: 'pass_clash' })}>PASS {stageLabel[pending.stage]}</button></div> : <div className="pk-clash-wait"><b>{view.players[pending.seat]?.name} IS RESPONDING</b><span>Hidden commitments remain private. Revealed cards and typed modifiers stay public.</span></div>}</>}
      </div>
    );
  }
...
function detectedIndustries(entry: CatalogEntry | undefined): string[] {
  if (entry?.structureVerified && entry.industries?.length) return [...entry.industries];
...
function playSpec(card: HandCard): Record<string, unknown> {
  ...
  const declaration = entry?.declarationVerified ? {
    capitalCost: entry.capitalCost ?? 0,
    carbonCost: entry.carbonCost ?? 0,
    corruptionRequirement: entry.corruptionRequirement ?? 0,
  } : {};
  if (card.kind === 'obligation' || entry?.type === 'obligation') return { kind: 'obligation', title };
  if (entry?.type === 'asset') return { kind: 'asset', title, ...declaration, industries: detectedIndustries(entry), startingMargin: typeof entry.margin === 'number' ? entry.margin : 0, corruption: entry.structureVerified ? entry.corruption : undefined };
  if (entry?.type === 'propaganda') {
    const text = `${entry.keywordsText ?? ''} ${entry.rulesText ?? ''}`.toLowerCase();
    const base = entry.bases?.[0] ?? BASES.find((candidate) => text.includes(candidate)) ?? 'capitalism';
    return { kind: 'propaganda', title, ...declaration, base, supportCost: entry.declarationVerified ? entry.supportCost ?? 0 : undefined, corruption: entry.structureVerified ? entry.corruption : text.includes('corruption'), negotiation: entry.structureVerified ? entry.negotiation : text.includes('negotiation') };
  }
  if (entry?.type === 'company') return { kind: 'company', title, ...declaration, industries: detectedIndustries(entry), startingMargin: typeof entry.margin === 'number' ? entry.margin : 1, corruption: entry.structureVerified ? entry.corruption : undefined };
  return { kind: 'event', title, ...declaration, corruption: entry?.structureVerified ? entry.corruption : undefined, edge: false };
}
...
function ActionPanel({ action, scene, view, me, boardTarget, targetRequest, setTargetRequest, showBroadcastPreview, send, focusHand }: {
  ...
  return (
    <div className="pk-action-detail">
      ...
      <div className="pk-visual-picker" data-testid="politik-clash-defender-picker"><span className="ig-lab">CHOOSE THE RIVAL NATION</span><div className="pk-nation-targets">{opponents.map((player) => <button key={player.seat} className={defender === player.seat ? 'on' : ''} onClick={() => { setDefender(player.seat); setDefenderCompany(player.companies[0]?.id ?? ''); }}><i style={{ background: SEAT_HEX[player.color] ?? player.color }} /><PolitikCard scene={scene} card={setupNationArt(player.nation)} label={player.name} /><b>{player.name}</b><small>{player.companies.length} COMPANIES</small></button>)}</div></div>
      ...
      {arena === 'corporate' && <div className="pk-company-duel"><div className="pk-visual-picker"><span className="ig-lab">YOUR COMPANY</span><div className="pk-card-targets">{me.companies.map((company) => <button key={company.id} className={attackerCompany === company.id ? 'on' : ''} onClick={() => setAttackerCompany(company.id)}><PolitikCard scene={scene} card={handCardArt(company.card)} label={company.title} /><b>{company.title}</b><small>MARGIN {company.margin}</small></button>)}</div></div><div className="pk-visual-picker"><span className="ig-lab">RIVAL COMPANY</span><div className="pk-card-targets">{targetCompanies.map((company) => <button key={company.id} className={defenderCompany === company.id ? 'on' : ''} onClick={() => setDefenderCompany(company.id)}><PolitikCard scene={scene} card={handCardArt(company.card)} label={company.title} /><b>{company.title}</b><small>MARGIN {company.margin}</small></button>)}</div></div></div>}
...
export function PolitikPlay({ view, act, error }: { view: PolitikView; act: (action: PolitikAction) => void; error: string | null }) {
  ...
  const focusKind = focused?.card.kind === 'politik' && focusMeta?.structureVerified ? focusMeta.type : focused?.card.kind === 'politik' ? focusDeclaredKind : typeof focusSpec?.kind === 'string' ? focusSpec.kind : '';
  ...
  const focusAllowedBases = focusMeta?.declarationVerified && focusMeta.bases?.length ? focusMeta.bases : BASES;
  ...
  const focusUnverified = focused?.card.kind === 'politik' && (!focusMeta?.structureVerified || focusMeta.margin === 'X');
      ...
      <main className={`pk-device-stage ${view.phase}`} data-pk-tour="board">
        ...
        {view.phase === 'playing' && (
          <aside className="pk-action-rail ig-glass" data-pk-tour="actions">
            ...
            <div className="pk-action-detail-wrap"><ActionPanel key={selectedAction ?? 'none'} action={selectedAction} scene={scene} view={view} me={me} boardTarget={boardTarget} targetRequest={targetRequest} setTargetRequest={setTargetRequest} showBroadcastPreview={setBroadcastPreview} send={send} focusHand={() => setViewer('hand')} /></div>
        ...
        <div className="pk-focus-backdrop" onClick={() => setFocused(null)}>
          <div className="pk-focus-card ig-glass" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="pk-focus-info">
              ...
              {focused.card.kind === 'politik' && focusMeta?.structureVerified && focusMeta.margin !== 'X' && <div className="pk-exact-card-data"><span className="ig-lab">VERIFIED PRINTED CARD DATA</span><p>{focusMeta.type.toUpperCase()} · {focusMeta.capitalCost ?? 0} CAPITAL · {focusMeta.carbonCost ?? 0} CARBON{focusMeta.corruptionRequirement ? ` · REQUIRES ${focusMeta.corruptionRequirement} CORRUPTION` : ''}</p><small>{focusMeta.industries?.length ? `INDUSTRIES: ${focusMeta.industries.map((industry) => industry.toUpperCase()).join(' / ')} · ` : ''}{typeof focusMeta.margin === 'number' ? `MARGIN ${focusMeta.margin} · ` : ''}{focusMeta.corruption ? 'CORRUPTION · ' : ''}{focusMeta.negotiation ? 'NEGOTIATION · ' : ''}Focus, declaration, and structural symbols are locked to multi-pass authentic-art review.</small></div>}
              ...
              {focusUnverified && <>
                <div className="pk-ocr-hint"><span className="ig-lab">OPTIONAL OCR TEXT · NEVER ENFORCED</span><p>{focusMeta?.rulesText ?? 'No transcription hint is available. Read the authentic card art.'}</p><small>{focusMeta?.declarationVerified ? `VERIFIED DECLARATION: ${focusMeta.capitalCost ?? 0} CAPITAL · ${focusMeta.carbonCost ?? 0} CARBON · MINIMUM CORRUPTION ${focusMeta.corruptionRequirement ?? 0}` : `COST HINT: ${focusMeta?.costText || 'UNAVAILABLE'}`}{focusMeta?.keywordsText ? ` · OCR KEYWORD HINT: ${focusMeta.keywordsText}` : ''}</small><b>Authentic art remains the authority. Only unresolved symbols are entered manually.</b></div>
                {focusMeta?.focus && <div className="pk-focus-values"><span className="pk-focus-hint-label">{focusMeta.focusVerified ? 'VERIFIED PRINTED FOCUS' : 'UNVERIFIED FOCUS HINT'}</span>{ARENAS.map((arena) => <span key={arena}><small>{arena}</small><b>{focusMeta.focus?.[arena] ?? 0}</b></span>)}</div>}
                <div className={`pk-manual-card-status${focusRequirementsConfirmed ? ' ready' : ''}`}>
                  <div><span className="ig-lab">{focusRequirementsConfirmed ? 'UNRESOLVED VALUES READY' : 'PARTIALLY VERIFIED CARD'}</span><b>{focusRequirementsConfirmed ? `${titleCase(focusKind)} · ${focusCost} CAPITAL · ${focusCarbonCost} CARBON` : 'CHECK ONLY THE SYMBOLS NOT YET VERIFIED'}</b><p>{focusRequirementsConfirmed ? 'Verified fields remain locked; your remaining printed-symbol choices will be sent to the rules engine.' : 'Focus, costs, requirements, Support, and Bases are already verified. Read the authentic art and enter only the unresolved type-specific symbols.'}</p></div>
                  <button data-testid="politik-card-manual-toggle" onClick={() => setFocusManualOpen((open) => !open)}>{focusManualOpen ? 'HIDE UNRESOLVED FIELDS' : focusRequirementsConfirmed ? 'EDIT UNRESOLVED FIELDS' : 'CHECK UNRESOLVED SYMBOLS'}</button>
                </div>
                {focusManualOpen && <div className="pk-manual-card-editor" data-testid="politik-card-manual-editor">
                  ...
                  {focusMeta?.declarationVerified ? <div className="pk-exact-card-data"><span className="ig-lab">VERIFIED PRINTED DECLARATION</span><p>{focusCost} CAPITAL · {focusCarbonCost} CARBON · MINIMUM CORRUPTION {focusCorruptionRequirement}</p><small>Two independent art reviews locked these values. They cannot be overridden by the client.</small></div> : <><ResourceStepper label="PRINTED CAPITAL COST" value={focusCost} max={Math.max(100, me.capital)} onChange={(value) => { setFocusCost(value); setFocusRequirementsConfirmed(false); }} /><ResourceStepper label="PRINTED CARBON COST" value={focusCarbonCost} max={10} onChange={(value) => { setFocusCarbonCost(value); setFocusRequirementsConfirmed(false); }} /><ResourceStepper label={`MINIMUM CORRUPTION REQUIRED (${me.corruption} CURRENT)`} value={focusCorruptionRequirement} max={20} onChange={(value) => { setFocusCorruptionRequirement(value); setFocusRequirementsConfirmed(false); }} /></>}
                  ...
                  {focusKind === 'propaganda' && <div className="pk-propaganda-requirements"><label>SUPPORT COMES FROM<select value={focusSupportBase} onChange={(event) => { setFocusSupportBase(event.target.value as BaseId); setFocusRequirementsConfirmed(false); }}>{focusAllowedBases.map((base) => <option key={base} value={base}>{base.toUpperCase()} · {me.support[base]} AVAILABLE</option>)}</select></label>{focusMeta?.declarationVerified ? <div className="pk-cost-line"><span>VERIFIED PRINTED SUPPORT COST</span><b>{focusSupportCost}</b></div> : <ResourceStepper label="PRINTED SUPPORT COST" value={focusSupportCost} max={10} onChange={(value) => { setFocusSupportCost(value); setFocusRequirementsConfirmed(false); }} />}<label>NEGOTIATION ICON<select value={focusNegotiation ? 'yes' : 'no'} onChange={(event) => { setFocusNegotiation(event.target.value === 'yes'); setFocusRequirementsConfirmed(false); }}><option value="no">NO NEGOTIATION</option><option value="yes">HAS NEGOTIATION</option></select></label></div>}
              ...
              {requirementsReady && focusKind === 'asset' && <div className="pk-visual-picker" data-testid="politik-asset-company-picker"><span className="ig-lab">PLACE ON A COMPANY</span><div className="pk-card-targets">{me.companies.map((company) => <button key={company.id} aria-pressed={focusTargetCompany?.id === company.id} className={focusTargetCompany?.id === company.id ? 'on' : ''} onClick={() => setFocusCompany(company.id)}><PolitikCard scene={scene} card={handCardArt(company.card)} label={company.title} /><b>{company.title}</b><small>MARGIN {company.margin} · {company.industries.map((industry) => industry.toUpperCase()).join(' / ')}</small></button>)}</div></div>}
              ...
              {requirementsReady && focusKind === 'company' && focusIndustries.length > 0 && <div className="pk-visual-picker" data-testid="politik-opening-market-picker"><span className="ig-lab">TAKE THE OPENING MARKET</span><div className="pk-token-targets">{focusIndustries.map((industry) => <button key={industry} aria-pressed={focusMarketIndustry === industry} className={focusMarketIndustry === industry ? 'on' : ''} disabled={view.marketSupply[industry] <= 0} onClick={() => setFocusIndustry(industry)}><i className={`pk-market-token ${industry}`} /><b>{industry.toUpperCase()}</b><small>{view.marketSupply[industry]} ON BOARD</small></button>)}</div><small>If every matching Market supply is empty, the Company enters without one.</small></div>}
              {requirementsReady && focusKind === 'asset' && focusTargetCompany && focusTargetCompany.margin + focusStartingMargin > 9 && <div className="pk-visual-picker" data-testid="politik-margin-overflow-picker"><span className="ig-lab">MARGIN PASSES 9 · CHOOSE THE PRINTED OVERFLOW</span><div className="pk-token-targets"><button aria-pressed={focusMarginMarket === 'remain'} className={focusMarginMarket === 'remain' ? 'on' : ''} onClick={() => setFocusMarginMarket('remain')}><i className="pk-margin-nine">9</i><b>STAY AT 9</b><small>NO MARKET</small></button>{focusTargetCompany.industries.map((industry) => <button key={industry} aria-pressed={focusMarginMarket === industry} className={focusMarginMarket === industry ? 'on' : ''} disabled={view.marketSupply[industry] <= 0} onClick={() => setFocusMarginMarket(industry)}><i className={`pk-market-token ${industry}`} /><b>{industry.toUpperCase()}</b><small>TAKE 1 · {view.marketSupply[industry]} LEFT</small></button>)}</div></div>}
              {requirementsReady && focusKind === 'propaganda' && me.propaganda.length >= 4 && <div className="pk-visual-picker" data-testid="politik-propaganda-replacement-picker"><span className="ig-lab">TAP THE PROPAGANDA TO REPLACE</span><div className="pk-card-targets">{me.propaganda.map((card) => <button key={card.instanceId} aria-pressed={focusReplacement === card.instanceId} className={focusReplacement === card.instanceId ? 'on' : ''} onClick={() => setFocusReplacement(card.instanceId)}><PolitikCard scene={scene} card={handCardArt(card.card)} label={card.title} /><b>{card.title}</b><small>{card.ready ? 'READY' : 'USED'}</small></button>)}</div></div>}
