...
<Panel id="panel" position="0 0 -220" rotation="90 270 90" scale="0.25 0.25">
<VerticalLayout id="bars" height="100">
  ...
  <Panel id="statePanel" height="150" width="-5" position="0 200 0">
    <VerticalLayout>
...
function onLoad(save_state)
  if save_state ~= "" then
    ...
    hDashboard_GUID = saved_data.hDashboard_GUID
    hDashboard = getObjectFromGUID(hDashboard_GUID)
...
function onSave()
  local save_state = JSON.encode({health = health, mana = mana, extra = extra, options = options, statNames = statNames,hDashboard_GUID = hDashboard_GUID})
...
function onClick(player, value, id)
  ...
  elseif statNames[id] ~= nil then
    ...
    if id == "add" then
        ...
        if hDashboard == nil then
            print("Sync with Hunter Dashboard")
        else
            hDashboard.call("hp_update_display", health)
        end
...
function onCollisionEnter(a)
    ...
    if newState == "Hunter Dashboard" then
        hDashboard_GUID = obj.getGUID()
        hDashboard = getObjectFromGUID(hDashboard_GUID)
    end
end

function updateHP(params)
    health.value = params.value
    self.UI.setAttribute("progressBar", "percentage", health.value / health.max * 100)
    self.UI.setAttribute("hpText", "text", health.value .. "/" .. health.max)
end
