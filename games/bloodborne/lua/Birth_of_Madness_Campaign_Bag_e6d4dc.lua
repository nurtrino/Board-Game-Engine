...
m4Pos = {position = {29.00, 10.30, -64.00}, rotation = {0.00, 0.00, 0.00}}
...
---------------Non Static Items-------------------------------------------------
        ...
        if name == "Mergo's Attendant" then
            params.guid = v.guid
            params.position = m3Pos.position
            params.rotation = m3Pos.rotation
            self.takeObject(params)
        end--end if
        if name == "Mergo's Chief Attendant" then
            params.guid = v.guid
            params.position = m4Pos.position
            params.rotation = m4Pos.rotation
            self.takeObject(params)
        end--end if
    end--end for
    self.destruct()
